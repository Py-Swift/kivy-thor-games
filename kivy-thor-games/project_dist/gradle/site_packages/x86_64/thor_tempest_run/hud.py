import os

from thorvg_cython import Scene, Text, Matrix

from .constants import COL_TEXT, COL_TEXT_MSG, Layout

_FONT_LOADED = False
_FONT_NAME   = 'Roboto-Bold'


def _ensure_font() -> None:
    global _FONT_LOADED
    if _FONT_LOADED:
        return
    _FONT_LOADED = True
    try:
        import kivy as _kivy
        path = os.path.join(os.path.dirname(_kivy.__file__), 'data', 'fonts', 'Roboto-Bold.ttf')
        Text.font_load(path)
    except Exception:
        pass


class Hud:
    """Score, speed, and centre message — pure thorvg Text objects."""

    def __init__(self, scene: Scene, layout: Layout) -> None:
        _ensure_font()
        self._scene = scene

        self._score_text = Text()
        self._speed_text = Text()
        self._msg_text   = Text()

        scene.add(self._score_text)
        scene.add(self._speed_text)
        scene.add(self._msg_text)

        self._score = 0
        self._build(layout)

    def _build(self, lo: Layout) -> None:
        self._lo  = lo
        pad = lo.hud_pad

        self._score_text.set_font(_FONT_NAME)
        self._score_text.set_size(lo.hud_font)
        self._score_text.set_color(*COL_TEXT)
        self._score_text.layout(lo.w * 0.4, lo.hud_font * 1.5)
        m = Matrix()
        m.e13 = pad
        m.e23 = pad
        self._score_text.set_transform(m)

        self._speed_text.set_font(_FONT_NAME)
        self._speed_text.set_size(lo.hud_font)
        self._speed_text.set_color(*COL_TEXT)
        self._speed_text.layout(lo.w * 0.35, lo.hud_font * 1.5)
        m2 = Matrix()
        m2.e13 = lo.w - lo.w * 0.35 - pad
        m2.e23 = pad
        self._speed_text.set_transform(m2)

        self._msg_text.set_font(_FONT_NAME)
        self._msg_text.set_size(lo.hud_msg_font)
        self._msg_text.set_color(*COL_TEXT_MSG)
        msg_w = lo.w * 0.8
        msg_h = lo.hud_msg_font * 2.5
        self._msg_text.layout(msg_w, msg_h)
        self._msg_text.align(0.5, 0.5)
        m3 = Matrix()
        m3.e13 = (lo.w - msg_w) / 2
        m3.e23 = lo.h * 0.45
        self._msg_text.set_transform(m3)

        self.set_score(self._score)
        self.hide_message()

    def rebuild(self, layout: Layout) -> None:
        self._build(layout)

    def set_score(self, n: int) -> None:
        self._score = n
        self._score_text.set_text(f'Score: {n}')

    def set_speed(self, s: float) -> None:
        self._speed_text.set_text(f'Speed: {int(s)}')

    def show_message(self, text: str) -> None:
        self._msg_text.set_text(text)
        self._msg_text.set_opacity(255)

    def hide_message(self) -> None:
        self._msg_text.set_opacity(0)
